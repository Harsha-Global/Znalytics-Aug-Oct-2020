...
app.controller("maincontent-controller", [
  ...
  "products-service",
  "products-factory",
  function ($scope, productsService, productsFactory) {
    $scope.pageTitle = "Products";

    //$scope.products = productsService.getProducts();

    //the getProducts() method returns a Promise
    productsFactory.getProducts().then(
      //The "resolve" function executes after receiving response with status code 200
      (response) => {
        console.log(response);
        $scope.products = response.data;
      },

      //The "reject" function executes after receiving response with status code 400 / 500
      (error) => {
        console.log(error);
      }
    );
...
//Create a factory in the ecommerce-module
app.factory("products-factory", [
  "$http",
  function ($http) {
    var getProd = function () {
      console.log("Inside getProducts");
      return $http.get("http://localhost:50580/api/Products");
    };
